"""Stage 2: LoRA fine-tuning of the DeepSeek backbone end-to-end."""
import argparse
import os
import sys
...
def main():
    ...
    parser.add_argument(
        "--warmup-checkpoint",
        required=True,
        help="Best warmup checkpoint to initialize from",
    )
    parser.add_argument("--resume", default=None, help="LoRA checkpoint to resume")
    ...
    model.set_trainable("warmup")  # Load all weights first.
    model.load_state_dict(torch.load(args.warmup_checkpoint)["model_state_dict"], strict=False)
